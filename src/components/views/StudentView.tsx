...
export default function StudentView({ activePage = "dashboard" }: { activePage?: string }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setRingOffset(283 - (283 * student.pulseScore) / 100);
    }, 100);
    return () => clearTimeout(timer);
  }, [student.pulseScore]);

  const scoreColor = student.pulseScore >= 75 ? "hsl(142 71% 45%)" : student.pulseScore >= 50 ? "hsl(25 95% 53%)" : "hsl(0 72% 51%)";
  ...
  const handleVideoAction = (title: string) => {
    ...
    if (status === "Idle") {
      ...
      setTimeout(() => {
        ...
        toast.success(`Completed: ${title}. Campus Pulse Score boosted!`);
  ...
  const [messages, setMessages] = useState<{role: "assistant"|"user", content: string}[]>([
    { role: "assistant", content: `Hi ${student.name.split(" ")[0]}! I'm CampusIQ. I can see your overall status is ${student.status}. How are you feeling about this semester?` }
  ]);
  ...
  const handleSendChat = async (e?: React.FormEvent) => {
    ...
    const systemPrompt = `You are CampusIQ, a warm empathetic AI academic counselor for a student at CampusIQ named ${student.name}. You know their academic data: Attendance ${student.attendance}%, IAT Total ${student.iat1+student.iat2}/100, Model ${student.model}/100, Campus Pulse Score ${student.pulseScore}/100. Speak like a supportive senior. Never lecture. Keep responses under 100 words. If distressed, gently suggest speaking to their mentor Dr. R. Meenakshi. Use Indian English phrasing.`;
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-10">
        ...
        <div className="grid gap-6 md:grid-cols-3 animate-fade-up stagger-1">
          ...
          <div className="col-span-1 border-border/50 card-warm card-glow-hover p-6 flex flex-col items-center justify-center text-center">
            <TooltipProvider>
              <UITooltip>
                <TooltipTrigger>
                  <h3 className="text-sm font-semibold text-muted-foreground w-full text-left mb-4 flex items-center gap-1 cursor-help hover:text-foreground transition-colors font-syne tracking-wide">CIT Campus Pulse Score ⓘ</h3>
                </TooltipTrigger>
            ...
            <div className="relative h-40 w-40">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center fade-slide-up" style={{ animationDelay: '0.3s' }}>
                <span className="text-5xl font-bold gradient-text-gold font-mono">{student.pulseScore}</span>
      ...
      {activePage === "chat" && (
      <div className="animate-fade-up">
        {/* Ask CampusIQ Assistant Chatbot */}
        <div className="card-warm hero-mesh flex flex-col h-[700px] overflow-hidden shadow-2xl max-w-4xl mx-auto rounded-[24px]">
          ...
          <div className="bg-surface/60 backdrop-blur-md border-b border-border/50 px-6 py-4 flex items-center gap-4 z-10">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl gradient-maroon shadow-[0_0_15px_hsl(0_85%_28%/_0.5)] bg-surface/50 border border-accent/20">
              <Shield className="h-5 w-5 text-accent animate-CampusIQ-beat" />
            </div>
            <div>
              <h3 className="font-bold gradient-text-gold font-syne text-lg tracking-wide">Ask CampusIQ</h3>
          ...
          <form onSubmit={handleSendChat} className="p-4 bg-surface-warm/80 backdrop-blur-md border-t border-border/50 flex gap-3 z-10 shadow-[0_-10px_40px_rgba(0,0,0,0.2)]">
            <input 
              ...
              disabled={isTyping}
              placeholder="Type your message to CampusIQ..."
