...
export default function HODView({ activePage = "dashboard" }: { activePage?: string }) {
  ...
  const kpis = [
    ...
    { id: "CampusIQ", label: "Avg Campus Pulse Score", val: summary.avgpulseScore, icon: Shield },
  ...
  const handleSendChat = async (e?: React.FormEvent) => {
    ...
    const systemPrompt = `You are CampusIQ, an AI HOD advisor for ${user?.name}. You analyze department performance. ${riskCounts.Critical} students are critical. Be concise, practical. Under 150 words.`;
  ...
  const runMacroAnalysis = async (pattern: string) => {
    ...
    const prompt = `You are CampusIQ an AI advisor for ${user?.name}, HOD of ${user?.department}. Analyze the macro pattern "${pattern}" currently affecting ${count} students in your department. Provide 1) Root causes at department level, 2) Strategic intervention policy to implement, 3) 1-month goal. Under 200 words.`;
  ...
  const handleTriggerScan = () => {
    toast("Running CampusIQ scan...", { duration: 2000 });
    setTimeout(() => {
      toast.success("CampusIQ scan complete — 0 new flags detected.");
    }, 2000);
    setFabOpen(false);
  };
            ...
            <div className="space-y-4">
              {["Attendance Collapse", "Hidden Talent Loss", "Disengagement", "Academic Overload"].map((p, idx) => (
                <div key={p} className="p-5 card-warm card-glow-hover border-border/50 transition-all" style={{ animationDelay: `${idx * 0.1}s`, animationFillMode: "both" }}>
                  ...
                  <p className="text-[11px] text-muted-foreground leading-relaxed">Macro pattern currently flagged in the cohort. CampusIQ Assistant can generate a targeted department strategy.</p>
                </div>
              ))}
            </div>
        ...
        <DialogContent className="sm:max-w-[500px] border-border/50 bg-surface-warm shadow-2xl p-6">
          ...
          <div className="py-4 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
            <div className="space-y-4 animate-fade-up">
              ...
              <p className="text-[13px] text-foreground my-2 leading-relaxed">The department has achieved a <strong className="text-chart-safe">6% increase</strong> in early retention due to the automated CampusIQ scaling policies.</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
