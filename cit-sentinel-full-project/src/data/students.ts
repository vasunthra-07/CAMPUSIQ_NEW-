export interface Student {
  ...
  driftType: "None" | "Lab Drift" | "Theory Drift" | "Full Dropout" | "Burnout";
...
function computeSentinelScore(attendance: number, iatTotal: number, lmsActivity: number, model: number, hackathonWins: number): number {
...
function computeDriftType(labAttendance: number, theoryAttendance: number, attendance: number, lmsActivity: number, iatTotal: number): Student["driftType"] {
  ...
  if (labAttendance < 60 && theoryAttendance < 60) return "Full Dropout";
...
function computeReasoningNote(
  ...
  attendance: number, weekTriggered: number, sentinelScore: number
): string {
  ...
  if (driftType === "Full Dropout") return `Both lab (${labAttendance}%) and theory (${theoryAttendance}%) attendance are below 60%. Multi-dimensional dropout risk.`;
  ...
  return `Performance stable. Sentinel score: ${sentinelScore}/100.`;
}
...
export function generateStudents(): Student[] {
  ...
  const docData: Array<Omit<Student, "status"|"persona"|"badges"|"sentinelScore"|"driftType"|"reasoningNote"|"weekTriggered"|"weeklyAttendance"|"interventionStatus">> = [
  ...
  for (const d of docData) {
    ...
    const sentinelScore = computeSentinelScore(d.attendance, iatTotal, d.lmsActivity, d.model, d.hackathonWins);
    ...
    const reasoningNote = computeReasoningNote(driftType, d.labAttendance, d.theoryAttendance, d.lmsActivity, iatTotal, d.hackathonWins, d.attendance, weekTriggered, sentinelScore);
    const interventionStatus = computeInterventionStatus(status, driftType);
    students.push({ ...d, status, persona, badges, sentinelScore, weeklyAttendance, weekTriggered, driftType, reasoningNote, interventionStatus });
  }
  ...
  for (let i = 10; i < 60; i++) {
    ...
    const sentinelScore = computeSentinelScore(attendance, iatTotal, lmsActivity, model, hackathonWins);
    ...
    const reasoningNote = computeReasoningNote(driftType, labAttendance, theoryAttendance, lmsActivity, iatTotal, hackathonWins, attendance, weekTriggered, sentinelScore);
    const interventionStatus = computeInterventionStatus(status, driftType);
    students.push({ id, name, department: dept, year, attendance, iat1, iat2, model, hackathonWins, lateSubmissions, lmsActivity, labAttendance, theoryAttendance, cgpa, competitionCount, status, persona, badges, sentinelScore, weeklyAttendance, weekTriggered, driftType, reasoningNote, interventionStatus });
  }
...
export const departmentStats = (() => {
  ...
  return depts.map(dept => {
    ...
    if (ds.length === 0) return { dept, total: 0, critical: 0, safe: 0, atRisk: 0, avgSentinel: 0, avgAttendance: 0, avgIAT: 0, avgLMS: 0, interventionRate: 0 };
    ...
    const avgSentinel = Math.round(ds.reduce((a, s) => a + s.sentinelScore, 0) / ds.length);
    ...
    return { dept, total: ds.length, critical, safe, atRisk, avgSentinel, avgAttendance, avgIAT, avgLMS, interventionRate };
  });
})();
