...
export function analyzeStudent(student: {
  ...
  if (student.hackathonWins >= 3 && iatTotal < 55) {
    return {
      ...
      sentinelScore: student.sentinelScore,
      riskLevel: "Moderate",
...
export function getBatchRiskSummary(students: any[]): {
  ...
  return {
    ...
    avgSentinelScore: Math.round(students.reduce((a, s) => a + s.sentinelScore, 0) / students.length),
    topPattern,
